...
HOWTO = [
    ('Ханоновский принцип', [
        'Каждое упражнение построено одинаково. Первый такт — фигура в до мажоре, '
        'второй — тот же такт для самопроверки. Дальше, после двойной черты, та же '
        'фигура идёт секвенцией по квинтовому кругу и через одиннадцать тональностей '
        'возвращается в до мажор. Тональность каждого такта подписана буквой над нотоносцем.'],),
    ('Большой палец и чёрные клавиши', [
        'Аппликатура в буги-вуги подчинена одному правилу: большой палец не играет по '
        'чёрной клавише. Исключения вынужденные — октавы и децимы, где верхний звук '
        'берётся большим независимо от цвета клавиши.',
        'Из этого правила следует, что аппликатура не переносится механически. В №1 '
        'большой встаёт на шестую ступень, и такая расстановка работает там, где эта '
        'ступень белая: C, F, B♭, E♭, A♭, D, G. В C♯, F♯, B, E и A шестая ступень чёрная, '
        'и расстановку приходится строить заново.',
        'Поэтому цифры проставлены только там, где решение однозначно: в №1 и в фигурах '
        'на октавах и децимах, где иного варианта нет. В остальных упражнениях аппликатура '
        'пока не выписана — лучше её отсутствие, чем неверная расстановка, вложенная в руку.']),
    ('Лестница темпов', [
